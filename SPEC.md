...
- [ ] Инициализировать проект: `npm init`, `tsconfig.json`, `package.json`, `Dockerfile`
- [ ] Настроить `docker-compose.yml` (api + db + minio + minio-init)
- [ ] Реализовать `config.ts` (Zod-валидация переменных окружения), `.env.example`
- [ ] Описать все модели в `prisma/schema.prisma`
- [ ] Создать первую миграцию Prisma (`prisma migrate dev`)
- [ ] Создать Fastify app (`app.ts`), зарегистрировать плагины (prisma, jwt, multipart, rateLimit)
- [ ] Настроить pino-логирование + глобальный error handler
- [ ] Реализовать `GET /health` (проверка БД через Prisma и S3 через head-запрос)
