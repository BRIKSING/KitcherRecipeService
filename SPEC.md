...
- [x] Проверить все коды ошибок по таблице 3.11
...
- [x] E2E тест: регистрация → создание рецепта → приготовление с жестами
- [x] Привести формат фото шагов к спецификации: `GET /recipes/:id/steps` теперь возвращает `url` + `thumb_url` вместо сырого `s3_key` (соответствие §3.7); обновлены тесты steps.test.ts и e2e.test.ts
- [x] Унифицировать формат фото шагов: `GET /recipes/:id` также возвращает `url` + `thumb_url` в объектах фото шагов (единообразие с `GET /recipes/:id/steps`); добавлены тесты в recipes.test.ts и e2e.test.ts
