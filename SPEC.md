...
- [d] Инициализировать проект: `npm init`, `tsconfig.json`, `package.json`, `Dockerfile`
- [d] Настроить `docker-compose.yml` (api + db + minio + minio-init)
- [d] Реализовать `config.ts` (Zod-валидация переменных окружения), `.env.example`
- [d] Описать все модели в `prisma/schema.prisma`
- [d] Создать первую миграцию Prisma (`prisma migrate dev`)
- [d] Создать Fastify app (`app.ts`), зарегистрировать плагины (prisma, jwt, multipart, rateLimit)
- [d] Настроить pino-логирование + глобальный error handler
- [d] Реализовать `GET /health` (проверка БД через Prisma и S3 через head-запрос)

### Этап 2 — Бэкенд: аутентификация

- [d] Описать Zod-схемы для auth (`schemas/auth.ts`)
- [d] Реализовать плагин `plugins/jwt.ts` (@fastify/jwt, декораторы sign/verify)
- [d] Реализовать `authService.ts` (register, login, refresh, logout)
- [d] Подключить bcrypt (`npm i bcrypt @types/bcrypt`)
- [d] Реализовать роутер `routes/auth.ts` (4 эндпоинта)
- [d] Реализовать middleware `authenticate.ts` и `isAdmin.ts`
- [d] Подключить rate limiting на `/auth/*` (@fastify/rate-limit)
- [d] Написать тесты `tests/auth.test.ts`

### Этап 3 — Бэкенд: рецепты, шаги, ингредиенты

- [d] Описать Zod-схемы для Recipe, Step, Ingredient (`schemas/`)
- [d] Реализовать `recipeService.ts` (CRUD + publish)
- [d] Реализовать `stepService.ts` (CRUD + reorder)
- [d] Реализовать роутер `routes/recipes.ts` (7 эндпоинтов)
- [d] Реализовать роутер `routes/steps.ts` (5 эндпоинтов)
- [d] Реализовать полнотекстовый поиск (PostgreSQL `tsvector` через Prisma `$queryRaw`)
- [d] Реализовать пагинацию и фильтрацию `GET /recipes`
- [d] Реализовать `GET /recipes/my` (черновики текущего пользователя)
- [d] Написать тесты `tests/recipes.test.ts`

### Этап 4 — Бэкенд: медиа и S3

- [d] Реализовать `storageService.ts` (@aws-sdk/client-s3: upload, delete, getSignedUrl)
- [d] Реализовать `utils/image.ts` (sharp: ресайз, EXIF autorotate, HEIC → JPEG)
- [d] Реализовать `photoService.ts` (upload, delete, reorder)
- [d] Реализовать роутер `routes/upload.ts` (`POST /upload/image`, @fastify/multipart)
- [d] Реализовать роутер `routes/photos.ts` (3 эндпоинта)
- [d] Настроить удаление S3-объектов при удалении рецепта/фото (Prisma middleware / хук)
- [d] Написать тесты `tests/upload.test.ts`
...
- [d] E2E тест: регистрация → создание рецепта → приготовление с жестами
- [d] Привести формат фото шагов к спецификации: `GET /recipes/:id/steps` теперь возвращает `url` + `thumb_url` вместо сырого `s3_key` (соответствие §3.7); обновлены тесты steps.test.ts и e2e.test.ts
- [d] Унифицировать формат фото шагов: `GET /recipes/:id` также возвращает `url` + `thumb_url` в объектах фото шагов (единообразие с `GET /recipes/:id/steps`); добавлены тесты в recipes.test.ts и e2e.test.ts
...
- [d] Финальный прогон тестов бэкенда
