...
## Этап 11 — Финальная интеграция: E2E-тест бэкенда

Сквозной (end-to-end) тест бэкенда, проверяющий весь путь пользователя
единым непрерывным сценарием: **регистрация → создание рецепта →
кулинарная сессия**. В отличие от модульных тестов Этапов 2–5, каждый из
которых покрывает свой роутер изолированно, этот тест проверяет
**стыковку этапов** — что артефакты одного шага (JWT-токены, идентификаторы
рецепта/шагов/фото) корректно используются на следующих. Соответствует
пункту «E2E тест» Этапа 11 SPEC.md (§5) и служит регрессионной страховкой
для всего бэкенд-API.

### Файл этапа

```
backend/tests/e2e.test.ts — сквозной сценарий из 15 последовательных шагов
```

### Что моделирует тест

Тест имитирует полный путь, который проходит мобильный клиент, работая
только с публичным HTTP-API (через `app.inject()`), без обращения к
внутренним сервисам напрямую:

```
register → login → create draft recipe → add steps →
upload image → attach photo → reorder steps → publish →
open recipe → fetch steps (cooking session) → refresh token → logout →
post-logout 401
```

Каждый из 15 `it`-блоков — отдельный этап сценария; они выполняются
**строго по порядку**, потому что результаты ранних шагов переиспользуются
поздними (см. ниже про токены).

### Стратегия моков

Тест намеренно занимает промежуточную позицию между «чистым» модульным и
полностью интеграционным:

- **Prisma, S3 (`@aws-sdk/client-s3`), bcrypt и `utils/image` — замоканы**
  (`vi.hoisted` + `vi.mock`), поэтому тест не требует реальных БД, хранилища
  и тяжёлой обработки изображений и остаётся быстрым и детерминированным;
- **JWT — настоящий** (`@fastify/jwt` не мокается). Это ключевой момент:
  access- и refresh-токены, полученные в ответах `register`/`login`,
  **реально подписаны** и затем предъявляются в заголовке
  `Authorization: Bearer …` на защищённых эндпоинтах. Так тест проверяет
  сквозную работу аутентификации (Этап 2), а не только отдельные обработчики.

Приложение поднимается один раз на весь файл через `buildApp()`
(`beforeAll`) и закрывается в `afterAll`; перед каждым шагом `beforeEach`
сбрасывает моки и переустанавливает базовые заглушки Prisma/S3/bcrypt.

### Переиспользование токенов между шагами

Переменные `accessToken` и `refreshToken` объявлены на уровне `describe` и
**протягиваются через весь сценарий**:

1. шаги 1–2 (`register`, `login`) захватывают токены из ответов;
2. шаги 3–10 подставляют `accessToken` в `Authorization` для создания
   рецепта, добавления шагов, загрузки/привязки фото, reorder и публикации;
3. шаг 13 (`refresh`) по `refreshToken` получает новый `accessToken`;
4. шаг 14 (`logout`) отзывает `refreshToken`;
5. шаг 15 подтверждает, что запрос без токена к защищённому
   `GET /recipes/my` возвращает **401** — сессия завершена.

Именно поэтому тесты последовательны (без `it.concurrent`): нарушение
порядка разорвало бы цепочку токенов.

### Покрываемые шаги сценария

| # | Запрос | Что проверяется |
|---|---|---|
| 1 | `POST /auth/register` | 201, наличие `access_token`/`refresh_token`, `user` без `password_hash` |
| 2 | `POST /auth/login` | 200, выдача новой пары токенов |
| 3 | `POST /recipes` | 201, черновик (`is_published: false`) с двумя ингредиентами |
| 4 | `POST /recipes/:id/steps` | 201, шаг с таймером `timer_sec: 480` |
| 5 | `POST /recipes/:id/steps` | 201, шаг без таймера (`timer_sec: null`) |
| 6 | `POST /recipes/:id/steps` | 201, третий шаг |
| 7 | `POST /upload/image` | 201, `{ url, thumb_url, key }`, две выгрузки в S3 (full + thumb) |
| 8 | `POST /steps/:step_id/photos` | 201, привязка фото, ответ с `url`/`thumb_url` |
| 9 | `PATCH /recipes/:id/steps/reorder` | 200, транзакционный reorder (`$transaction` вызван) |
| 10 | `POST /recipes/:id/publish` | 200, `is_published: true` |
| 11 | `GET /recipes/:id` | 200, полный рецепт для кулинарной сессии: 3 шага, таймер, фото |
| 12 | `GET /recipes/:id/steps` | 200, упорядоченные шаги для пошаговой навигации |
| 13 | `POST /auth/refresh` | 200, новый `access_token` без `refresh_token` |
| 14 | `POST /auth/logout` | 204, `RefreshToken.revoked = true` |
| 15 | `GET /recipes/my` | 401 без токена — доступ после выхода закрыт |

### Проверка формата фото шагов (§3.7)

Шаги 11 и 12 явно проверяют, что фото шага возвращаются в едином формате
**`{ url, thumb_url, sort_order }`** и **не** содержат сырого `s3_key` — как
в `GET /recipes/:id`, так и в `GET /recipes/:id/steps`. Это фиксирует
результат двух смежных пунктов Этапа 11 (приведение и унификация формата
фото шагов) и защищает соответствие §3.7 от регрессий: клиент всегда
получает готовые ссылки на полное изображение и миниатюру, а не внутренний
ключ хранилища.

### Роль в наборе тестов

E2E-тест дополняет, а не заменяет модульные тесты Этапов 2–5: он не проверяет
исчерпывающе все коды ошибок каждого эндпоинта, но гарантирует, что «счастливый
путь» целиком проходит через все слои (auth → recipes → steps → media) и что
контракты между ними согласованы. Запускается вместе со всем бэкенд-сьютом
через `npm run test` (Vitest).

---

## Этап 11 — Формат фото шагов: `GET /recipes/:id/steps` → `url` + `thumb_url` (§3.7)

Приведение ответа `GET /recipes/:id/steps` к спецификации §3.7: фотографии
шага возвращаются в публичном виде **`{ id, url, thumb_url, sort_order }`**
вместо сырого `s3_key`. Раньше эндпоинт отдавал строку внутреннего ключа
объекта S3 (`s3_key`), из-за чего клиент был вынужден сам склеивать URL и
угадывать имя миниатюры — это протекание деталей хранилища в контракт API.
Теперь сервер отдаёт готовые ссылки на полное изображение и на превью.

### Файлы этапа

```
backend/src/services/stepService.ts — helpers formatStep / formatStepPhoto (сборка url/thumb_url)
backend/src/routes/steps.ts         — все GET/POST/PUT/PATCH возвращают шаги в новом формате
backend/tests/steps.test.ts         — регрессия на формат фото (url/thumb_url, отсутствие s3_key)
backend/tests/e2e.test.ts           — проверка формата в шаге «кулинарная сессия»
```

### Где строится представление фото

Единственная точка преобразования — `stepService.ts`. Каждый шаг перед
отправкой прогоняется через `formatStep`, а каждое фото — через
`formatStepPhoto`:

| Поле ответа | Источник | Как получается |
|---|---|---|
| `id` | `StepPhoto.id` | как есть |
| `url` | `StepPhoto.s3_key` | `${S3_PUBLIC_URL}/${s3_key}` |
| `thumb_url` | `StepPhoto.s3_key` | тот же URL с заменой `/full.jpg` → `/thumb.jpg` |
| `sort_order` | `StepPhoto.sort_order` | как есть |

Деривация `thumb_url` детерминирована, потому что ключи изображений всегда
имеют вид `images/{uuid}/full.jpg` (см. Этап 4, `utils/image.ts` и
`photoService`), а рядом лежит `images/{uuid}/thumb.jpg`. Отдельного столбца
для миниатюры в БД нет — она вычисляется из full-ключа, и это единственная
причина, по которой `s3_key` вообще хранится в форме `.../full.jpg`.

### Единый формат во всех ответах со шагами

`formatStep` применяется во **всех** методах сервиса, возвращающих шаг —
`findByRecipeId` (GET-список), `create`, `update`, `reorder`. Поэтому клиент
получает один и тот же формат фото независимо от того, читает он шаги,
создаёт или переупорядочивает их; сырой `s3_key` не утекает ни в одном из
ответов. Тот же способ построения URL используется в `recipeService` для
`GET /recipes/:id`, что гарантирует единообразие между двумя эндпоинтами,
отдающими шаги.

### Роль роутера

`routes/steps.ts` бизнес-логику формата не содержит — он лишь вызывает
сервис и транслирует результат. `GET /recipes/:id/steps` открыт без
аутентификации (просмотр рецепта), пишущие эндпоинты требуют
`Authorization: Bearer <access_token>` и проверяют владельца рецепта внутри
сервиса. Роут `/reorder` регистрируется раньше `/:step_id`, иначе литерал
`reorder` был бы сматчен как идентификатор шага.

### Тесты

- `steps.test.ts` — на ответе `GET /recipes/:id/steps` с одним фото
  проверяется: наличие `url` и `thumb_url`, **отсутствие** `s3_key`, что
  `url` указывает на `.../full.jpg`, а `thumb_url` — на `.../thumb.jpg`, и
  сохранение `sort_order`.
- `e2e.test.ts` — шаг «кулинарная сессия» (`GET /recipes/:id/steps`)
  подтверждает тот же контракт в сквозном сценарии.

Стадия проверена: правок логики не потребовалось, весь бэкенд-сьют проходит.
В §5 SPEC.md пункт отмечен как `[d]` (задокументировано).

---

## Этап 11 — Унификация формата фото шагов в `GET /recipes/:id` (§3.7)

Продолжение предыдущего пункта. Ранее к спецификации §3.7 был приведён
только эндпоинт `GET /recipes/:id/steps` (сборка формата в `stepService.ts`).
Однако полный рецепт отдаёт другой сервис — `recipeService.ts`, — и его метод
`GET /recipes/:id` возвращал фотографии шагов **в старом виде** (сырой
`s3_key`). Возникло расхождение: один и тот же объект «фото шага» выглядел
по-разному в двух эндпоинтах, отдающих шаги. Данный этап устраняет это
расхождение — теперь **оба** эндпоинта отдают фото в едином публичном виде
**`{ id, url, thumb_url, sort_order }`** без утечки внутреннего ключа S3.

### Файлы этапа

```
backend/src/services/recipeService.ts — helper buildThumbUrl + формат photos в formatRecipe
backend/tests/recipes.test.ts         — регрессия на формат фото в GET /recipes/:id
backend/tests/e2e.test.ts             — проверка формата в шаге «открыть рецепт» (cooking session)
```

Роутер `routes/recipes.ts` при этом **не менялся**: он лишь вызывает сервис и
транслирует результат, вся логика формата сосредоточена в сервисе.

### Что изменилось в `recipeService.ts`

Формат фото собирается в единственной функции `formatRecipe`, через которую
проходят **все** ответы сервиса, содержащие шаги: `findById`
(`GET /recipes/:id`), `findAll` (`GET /recipes`), `findMy`
(`GET /recipes/my`), а также `create`, `update`, `publish`. Поэтому правка в
одном месте автоматически унифицирует формат во всех этих эндпоинтах.

Внутри `formatRecipe` каждый шаг разворачивается, и его фото преобразуются
из строки `s3_key` в объект с готовыми ссылками:

```ts
photos: step.photos.map((photo: any) => ({
  id: photo.id,
  url: buildImageUrl(photo.s3_key),                       // ${S3_PUBLIC_URL}/${s3_key}
...
|---|---|---|
| `id` | `StepPhoto.id` | как есть |
| `url` | `StepPhoto.s3_key` | `${S3_PUBLIC_URL}/${s3_key}` (helper `buildImageUrl`) |
| `thumb_url` | `StepPhoto.s3_key` | тот же URL с заменой `/full.jpg` → `/thumb.jpg` (helper `buildThumbUrl`) |
| `sort_order` | `StepPhoto.sort_order` | как есть |

Добавлен локальный helper `buildThumbUrl`, повторяющий логику из
`stepService.ts`: миниатюра выводится из full-ключа детерминированной заменой
`/full.jpg` → `/thumb.jpg`. Это возможно потому, что ключи изображений всегда
имеют вид `images/{uuid}/full.jpg` (см. Этап 4, `utils/image.ts` и
`photoService`), а рядом всегда лежит `images/{uuid}/thumb.jpg`. Отдельного
столбца под миниатюру в БД нет — она вычисляется, а не хранится.

### Почему логика продублирована, а не вынесена в общий модуль

`recipeService` и `stepService` строят URL одинаково, но каждый держит свою
пару helper'ов (`buildImageUrl`/`buildThumbUrl`). Это осознанный компромисс:
функции тривиальны (конкатенация и одна замена подстроки), а связывать два
сервиса общим модулем ради двух строк — избыточная связанность. Инвариант,
который нужно удерживать при изменениях, зафиксирован в комментарии над
`buildThumbUrl` в `recipeService.ts` и в шапке `stepService.ts`: **оба сервиса
обязаны выводить один и тот же формат фото шага**; регрессионные тесты (ниже)
стерегут это соответствие.

### Обработка граничных случаев

- **Шаг без фото** — `step.photos` пустой массив, `.map` даёт `[]`; поле
  `photos` всегда присутствует.
- **`s3_key` пустой/`null`** — `buildImageUrl` вернёт `null` для `url`, а
  `thumb_url` защищён проверкой `photo.s3_key ? … : null`. В нормальном потоке
  такого не бывает (фото привязывается только с валидным ключом), но формат
  ответа остаётся согласованным (оба поля — `null`), а не падает.

### Тесты

- `recipes.test.ts` — тест «returns step photos with url AND thumb_url» на
  ответе `GET /recipes/:id` с одним фото проверяет: наличие `url` и
  `thumb_url`, **отсутствие** `s3_key`, что `url` указывает на `.../full.jpg`,
  `thumb_url` — на `.../thumb.jpg`, и сохранение `sort_order`.
- `e2e.test.ts` — шаг 11 «GET /recipes/:id (открыть рецепт для кулинарной
  сессии)» подтверждает тот же контракт в сквозном сценарии; вместе со
  смежным шагом 12 (`GET /recipes/:id/steps`) он фиксирует, что **оба**
  эндпоинта отдают идентичный формат фото.

Итог: контракт §3.7 теперь единообразно выдержан во всех ответах бэкенда,
отдающих шаги рецепта; клиент всегда получает готовые ссылки на полное
изображение и миниатюру, а внутренний ключ хранилища наружу не протекает.

---

## Этап 11 — Финальный прогон тестов бэкенда

Завершающий шаг бэкенд-части (§5, Этап 11): единый прогон всего набора тестов,
подтверждающий, что все реализованные ранее этапы (2–5 и интеграционные
доработки Этапа 11) работают вместе и ни один из них не сломал соседние. Это
не отдельный код-артефакт, а операция верификации над всем набором
`backend/tests/*` через один вход — `npm test`.
...
npx prisma generate  # генерация Prisma Client (типы для @prisma/client)
npm test             # прогон всего набора: vitest run
```

Скрипт `test` в `package.json` — это `vitest run` (одноразовый прогон без
watch-режима; для разработки есть `test:watch` → `vitest`). Конфигурация в
`vitest.config.ts`:

```ts
export default defineConfig({
  test: {
    setupFiles: ['./tests/setup.ts'],  // env-переменные до загрузки config.ts
    environment: 'node',
...
### Тестовое окружение (`tests/setup.ts`)

`setupFiles` выполняется **до** загрузки любого модуля, поэтому здесь
выставляются все обязательные переменные окружения (`DATABASE_URL`,
`JWT_SECRET`, `S3_*`, `PORT`, `CORS_ORIGIN`, `RATE_LIMIT_PER_MINUTE` и т. д.).
Это критично: `config.ts` валидирует env через Zod при импорте и упал бы без
этих значений. Значения фиктивные — реальные Postgres и S3 в тестах не нужны
(см. стратегию моков ниже). `RATE_LIMIT_PER_MINUTE=1000` намеренно завышен,
чтобы rate-limit не отбрасывал запросы во время быстрых прогонов.

### Стратегия моков (общая для всего набора)

Тесты не требуют внешней инфраструктуры — они полностью изолированы:

| Зависимость | В тестах | Зачем |
|---|---|---|
| Prisma (`@prisma/client`) | замокан (`vi.mock`) | нет реальной БД; каждый тест задаёт ожидаемые ответы через `mockResolvedValue` |
| S3 (`@aws-sdk/client-s3`) | замокан | `send()` возвращает `{}`; проверяется факт и число вызовов |
| `bcrypt` | замокан | `hash`/`compare` детерминированы, без реального хэширования |
| `utils/image` (sharp) | замокан | `processImage` возвращает фиксированные ключи/буферы |
| JWT (`@fastify/jwt`) | **реальный** | токены из auth-ответов действительно подписываются/проверяются и работают в последующих запросах |

Каждый файл поднимает настоящий Fastify-инстанс через `buildApp()` и гоняет
запросы через `app.inject(...)` (in-process, без сетевого сокета). То есть
проверяется реальная цепочка «роутинг → плагины → middleware → сервис», а на
границе с БД/S3/криптографией стоят моки.

### Состав набора и что покрывает каждый файл

| Файл | Тестов | Покрытие |
|---|---|---|
| `tests/auth.test.ts` | 19 | `POST /auth/register`, `/login`, `/refresh`, `/logout` (Этап 2) |
| `tests/recipes.test.ts` | 42 | CRUD рецептов, `GET /recipes` (фильтры/пагинация/поиск), `/recipes/my`, `/publish`, а также шаги в контексте рецепта (Этап 3) |
| `tests/steps.test.ts` | 36 | Все 5 эндпоинтов шагов: `GET/POST/PUT/DELETE /recipes/:id/steps`, `PATCH .../reorder` (Этап 3) |
| `tests/upload.test.ts` | 17 | `POST /upload/image`, `POST/DELETE /steps/:id/photos`, `PATCH .../photos/reorder` (Этап 4) |
| `tests/categories.test.ts` | 12 | `GET /categories`, `GET /categories/:id`, admin-only `POST`, `DELETE` (Этап 5) |
| `tests/tags.test.ts` | 8 | `GET /tags` (с поиском `?q=`), `POST /tags` (Этап 5) |
| `tests/e2e.test.ts` | 15 | Сквозной сценарий регистрация → создание рецепта → кулинарная сессия (Этап 11) |
| **Итого** | **149** | Все эндпоинты бэкенда из §3.5 |

### Результат финального прогона

```
 ✓ tests/e2e.test.ts        (15 tests)
 ✓ tests/recipes.test.ts    (42 tests)
 ✓ tests/steps.test.ts      (36 tests)
 ✓ tests/upload.test.ts     (17 tests)
 ✓ tests/auth.test.ts       (19 tests)
 ✓ tests/categories.test.ts (12 tests)
 ✓ tests/tags.test.ts       (8 tests)
...
Все 7 файлов и все 149 тестов проходят зелёными; фейлов и пропусков нет.
Прогон занимает ~2 с за счёт in-process `app.inject` и полностью замоканных
внешних зависимостей — реальные Postgres/S3/сеть не поднимаются.

### Роль этапа

Этот прогон — «gate» перед завершением бэкенда: он гарантирует, что доработки
формата фото шагов (§3.7) и E2E-сценарий Этапа 11 не сломали набор тестов
предыдущих этапов, и что весь публичный контракт API из §3.5 покрыт и
проверен единым запуском `npm test`.
