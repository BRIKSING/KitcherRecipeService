/**
 * Сервис шагов рецепта (§3.5) + приведение формата фото к спецификации
 * (§3.7, Этап 11).
 *
 * Бизнес-логика CRUD и reorder шагов, а также единственная точка сборки
 * публичного представления фото шага. В БД `StepPhoto` хранит только
 * `s3_key` (внутренний ключ объекта S3); наружу этот ключ отдавать нельзя —
 * клиенту нужны готовые ссылки. Поэтому каждый шаг перед отправкой
 * прогоняется через `formatStep`, а каждое фото — через `formatStepPhoto`,
 * который выводит `{ id, url, thumb_url, sort_order }`.
 *
 * Ключи изображений имеют вид `images/{uuid}/full.jpg` (см. `utils/image.ts`
 * и `photoService`); отсюда `thumb_url` детерминированно выводится заменой
 * `/full.jpg` → `/thumb.jpg`, а сам URL — как `${S3_PUBLIC_URL}/${s3_key}`.
 * Тот же способ построения используется в `recipeService` (для `GET
 * /recipes/:id`), что гарантирует единый формат фото шага в обоих ответах.
 */
